<?php
	...
	$sql = "SELECT * from product_subcategories where subcategorie='".$sub_categorie."' and id!=:id"; 
	...
	// ,status=:status_p,udate=GETDATE(),modify_by=:modify_by_p
	$sql="update product_subcategories set subcategorie=:subcategorie_p,id_product_categorie=:id_product_categorie_p,status=:status_p,udate=GETDATE(),modify_by=:modify_by_p where id=:id";
	...
	if($_GET['id']){
		$sql_user = " SELECT * from product_subcategories WHERE id=".$_GET['id'];
		$query_user = $dbh->prepare($sql_user);
		$query_user->execute();
		$results_user = $query_user->fetchAll(PDO::FETCH_OBJ);
		foreach($results_user as $result){ 
			$sub_categorie = $result->subcategorie;
			$categorie = $result->id_product_categorie;
                    ...
                    <div class="form-group row col-md-10 col-md-offset-1">
                        <div class="col">
                            <div class="panel-title" style="color: #102958;" >
                                <h2 class="title">Edit Sub Categories</h2>
                            </div>
                        </div>
                        <div class="col-sm-2 ">
                        </div>
