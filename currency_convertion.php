...
<?php
	...
	/*
	$sql = "SELECT cl.currency AS currency_code
	,cl_con.currency AS currency_code_convert
	,FORMAT(cc.start_date, 'dd-MM-yyyy') as start_date_f
	,FORMAT(cc.stop_date, 'dd-MM-yyyy') as stop_date_f
	,cc.* FROM  currency_convertion cc
	LEFT JOIN currency_list cl ON cc.id_currency_list = cl.id
	LEFT JOIN currency_list cl_con ON cc.id_currency_list_convert = cl_con.id
	...
	*/
		...
		// $showAll= isset($_GET['status']) && $_GET['status'] == 'true';

		// Prepare SQL query with status condition
		$sql = "SELECT cl.currency AS currency_code
				,cl_con.currency AS currency_code_convert
				,FORMAT(cc.start_date, 'dd-MM-yyyy') AS start_date_f
				,FORMAT(cc.stop_date, 'dd-MM-yyyy') AS stop_date_f
			...
			<div class="row breadcrumb-div" style="background-color:#ffffff">
				<div class="col-md-6" >
					<ul class="breadcrumb">
						...
						<!-- <li> Classes</li> -->
						<li class="active" >Currency Conversions</li>
					</ul>
				</div>
			</div>
        </div>
			...
			<div class="card shadow mb-4">
				<div class="card-header py-3">
					<div class="panel-title" style="display: inline-block;">
						<h2 class="title m-5" style="color: #102958;">Currency Conversions</h2>
					</div>
...
<!------------------------------------------------------------------------------->
		...
		<div class="card-body" >
			<div class="table-responsive" style="font-size: 13px;">
				...
				<!--<input id="status" name="status" style="" class="form-check-input" type="checkbox" value="true" checked>-->
				<input id="status" name="status" style="" class="form-check-input" type="checkbox" <?php if($_GET['status']=='true'){ echo "checked"; } ?> >
					&nbsp;&nbsp;&nbsp;&nbsp; Show All
...
//         form.method = "POST";
//         form.action = "currency_convertion.php";
//         form.appendChild(createHiddenField("status", "true"));
				...
				<table id="example"  class="table table-bordered"  style="color: #969FA7; font-size: 13px;" width="100%"  >
					<thead >
						<tr style="color: #102958;" >
							...
							<th style="color: #102958;">Currency Value</th>
							<th style="color: #102958;">Currency Code</th>
							
							<th style="color: #102958;">Convert to Value</th>
							<th style="color: #102958;" >Convert to Currency</th>
					...
					<tr>
						...
						<td class="id_table" hidden="true" ><?php echo $result->id;?></td>
						<td class="text-center" class="currency_value" ><?php echo number_format((float)$result->currency_value, 2, '.', ''); ?></td>
						<td class="text-center" class="currency_code" ><?php echo $result->currency_code;?></td>

						<td class="text-center" class="currency_table" ><?php echo number_format((float)$result->currency_value_convert, 2, '.', '');?></td>
						<td class="text-center" class="currency_table" ><?php echo $result->currency_code_convert;?></td>

						<td class="text-center" class="start_date_table" ><?php echo $result->start_date_f;?></td>
