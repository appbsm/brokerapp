<?php
	...
	$sql = "SELECT pd.*,pc.categorie AS categorie_name,ps.subcategorie AS subcategorie_name FROM product pd 
	LEFT JOIN product_categories pc ON pc.id = pd.id_product_categories
	LEFT JOIN product_subcategories ps ON ps.id = pd.id_product_sub ORDER BY LTRIM(product_id) asc ";
	// ORDER BY LTRIM(product_name) asc
	$query = $dbh->prepare($sql);
	$query->execute();
...
<body id="page-top" >
			...
			<div class="card shadow mb-4">
				<div class="card-header py-3">
					<div class="panel-title" style="display: inline-block;">
						<h2 class="title m-5" style="color: #102958;">Products</h2>
					</div>
    ...
    var table = $('#example').DataTable({
        ...
        "paging":         true,
        buttons: [
            { extend: 'csv',class: 'buttons-csv',className: 'btn-primary',charset: 'UTF-8',filename: 'Product',bom: true
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
            { extend: 'excel',class: 'buttons-excel', className: 'btn-primary',charset: 'UTF-8',filename: 'Product',bom: true 
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
            { extend: 'pdf',class: 'buttons-pdf',className: 'btn-primary',charset: 'UTF-8',filename: 'Product',bom: true 
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
