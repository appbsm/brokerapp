<?php
	...
	if($_GET['id']){
		...
		if(count($results_insurance)==0){
			$sql="delete from customer_level where id=:id";
			$query = $dbh->prepare($sql);
...
<body id="page-top" >
			...
			<div class="row breadcrumb-div" style="background-color:#ffffff">
				<div class="col-md-6" >
					<ul class="breadcrumb">
						...
						<li class="active" >Customer Level</li>
					</ul>
				</div>
			</div>
        </div>
			...
			<div class="card shadow mb-4">
				<div class="card-header py-3">
					<div class="panel-title" style="display: inline-block;">
						<h2 class="title m-5" style="color: #102958;">Customer Level</h2>
					</div>
						...
						<table id="example"  class="table table-bordered "  style="color: #969FA7;" width="100%" >
							<thead >
								<tr style="color: #102958;" >
									<th class="text-center" width="50px" style="color: #102958;">#</th>
									<th class="text-center" style="color: #102958;">Level Code</th>
									<th class="text-center" style="color: #102958;">Level Description</th>
									<th class="text-center" width="90px" style="color: #102958;">Status</th>
									<th class="text-center" width="90px" style="color: #102958;">Action</th>
								</tr>
							</thead>
							...
							?> 
								<tr>
									...
									<td class="text-center"><?php echo $result->level_name;?></td>
    ...
    var table = $('#example').DataTable({
        ...
        "paging":         true,
        buttons: [
            { extend: 'csv',class: 'buttons-csv',className: 'btn-primary',charset: 'UTF-8',filename: 'Customer Level',bom: true
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
            { extend: 'excel',class: 'buttons-excel', className: 'btn-primary',charset: 'UTF-8',filename: 'Customer Level',bom: true 
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
            { extend: 'pdf',class: 'buttons-pdf',className: 'btn-primary',charset: 'UTF-8',filename: 'Customer Level',bom: true 
            ,exportOptions: {columns: ':not(:last-child)'},init : function(api,node,config){ $(node).hide();} },
