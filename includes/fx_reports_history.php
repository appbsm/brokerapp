...
function get_reports_history_start($conn) {
 ...
 JOIN (SELECT MAX(rehi.id) AS id_last,rehi.id_insurance_info,rehi.status,rehi.paid_date
 ,MAX(rehi.start_date) AS start_date
 FROM report_history_policy rehi 
 GROUP BY rehi.policy_no,rehi.id_insurance_info,rehi.status,rehi.paid_date) rea ON hist.id = rea.id_last
 LEFT JOIN customer cu ON cu.id = hist.customer_id
 LEFT JOIN contact con ON con.id = hist.contact_id_default
...
function get_reports_history_search($conn,$post_data) {
 ...
 OR cc.id_currency_list_convert    = cl.id 
 AND  info.paid_date >= c_c.start_date and info.paid_date <= c_c.stop_date)
 LEFT JOIN user_info u_up ON u_up.id = hist.modify_by where hist.id IS NOT NULL ";
